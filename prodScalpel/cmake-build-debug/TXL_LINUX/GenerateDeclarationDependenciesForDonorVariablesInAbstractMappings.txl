...
define dependency_output
	'FUNCTION_NAME: [NL] [inLineComment]@[repeat id] [NL] 'DECLARATION_LIST: [NL] [repeat inline_comment_with_line_end]  [end_marker] [NL]
end define
...
function main
    ...
    deconstruct TXLargs
    	neededMarkersFile [stringlit] outputFile [stringlit]
	deconstruct P 
		interfaceCompUnit [compilation_unit]
		
	%construct the repeat of needed markers
	construct neededMarkers [repeat inLineComment]
		_ [read neededMarkersFile]
    ...
    construct createTheListOfStatements [function_definition_or_declaration]
   		TheInterfaceFunction [fixEveryBlock TheInterfaceFunction neededMarkers]		
...
rule fixEveryBlock	TheInterfaceFunction [function_definition_or_declaration]
					NeededMarkers [repeat inLineComment]
	...
	construct addStatementsFromDecl [compound_statement_body]
		currentBlockBody [matchStatementsFromDeclaration currentBlock TheInterfaceFunction NeededMarkers]
	by
		currentBlock
end rule
...
rule matchStatementsFromDeclaration	CurrentBlock [compound_statement]
	...
	where
		NeededMarkers [matchCurrentMarker StatementNumberMarker]
	...
	export CurrentMatchedIDs [repeat id]
		_
	...
	import CurrentMatchedIDs [repeat id] 
	%don't add decl/statements for which there is no dependency
	construct dependOutputElNonEmpty [repeat dependency_output]
		_ [constructGlobalDependencyListWhenNonEmpty StatementNumberMarker repeatOfNeededDecl CurrentMatchedIDs]
...
rule matchCurrentMarker	SearchedMarker [inLineComment]
	match [inLineComment]
...
function constructGlobalDependencyListWhenNonEmpty	CommentMarker [inLineComment]
	...
	construct newNeededDependenciesForTypeDefs [dependency_output]
		'FUNCTION_NAME: CommentMarker@MatchedIDs 'DECLARATION_LIST: IdentifiedDecl endMarker
...
function addDeclIfCaseFromBaseDecl	CurrentDeclaration [declaration]
	...
	construct currentDeclMarkerNL [inline_comment_with_line_end]
		currentDeclMarker
	where
		listOfIDs [matchAnIdFromTheList ListOfNeededIDs CurrentStatementDeclForWhichIsSearchedDependency currentDeclMarker]
	where not
...
function addDeclIfCaseFromTypeSpecifier	CurrentDeclaration [declaration]
	...
	where
		lastID [matchAnIdFromTheList ListOfNeededIDs CurrentStatementDeclForWhichIsSearchedDependency currentDeclMarker]
	where not
		CurrentStatementDeclForWhichIsSearchedDependency [= currentDeclMarker]
...
function matchAnIdFromTheList	ListOfNeededIDs [repeat id]
								SearchedDeclMarker [inLineComment]
								CurrentDeclMarker [inLineComment]
	match * [id]
		current [id]
	where
		ListOfNeededIDs [matchInList current SearchedDeclMarker CurrentDeclMarker]
end function
...
function matchInList	CurrentID[id]
	...
	where
		current [= CurrentID]
	where not	
		SearchedDeclMarker [= CurrentDeclMarker]
		
		import CurrentMatchedIDs [repeat id]
		export CurrentMatchedIDs 
			CurrentMatchedIDs [.CurrentID]
